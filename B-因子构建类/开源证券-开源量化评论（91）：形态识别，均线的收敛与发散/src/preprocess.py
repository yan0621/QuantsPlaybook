'''
Author: Hugo
Date: 2024-03-11 13:25:08
LastEditors: error: error: git config user.name & please set dead value or install git && error: git config user.email & please set dead value or install git & please set dead value or install git
LastEditTime: 2024-03-11 13:25:13
Description: 因子标准化，缩尾等处理方法
'''
...
from .utils import affirm, ignore_warning


@ignore_warning(message="Mean of empty slice", category=RuntimeWarning)
@ignore_warning(message="Degrees of freedom <= 0 for slice", category=RuntimeWarning)
@ignore_warning(
    message="invalid value encountered in true_divide", category=RuntimeWarning
)
...
def winsorize(
    ...
    if qrange:
        affirm(
            (0 <= qrange[0] <= 1) and (0 <= qrange[1] <= 1),
            "qrange 值应在 0 到 1 之间，如 [0.05, 0.95]",
        )
        qrange = (Decimal(str(qrange[0])), 1 - Decimal(str(qrange[1])))

        if inclusive:
            v[~np.isnan(v)] = spwinsorize(
                v[~np.isnan(v)], qrange, inclusive=[True, True]
            )
        else:
            # 如果v是int arrary，无法给 array 赋值 np.nan，因为 np.nan 是个 float
            v = v.astype(float)
            not_nan = v[~np.isnan(v)]
            not_nan[not_nan != spwinsorize(not_nan, qrange, inclusive=[True, True])] = (
                np.nan
            )
        ...
        if range:
            range_ = (
                Decimal(str(range[0])) if not np.isnan(range[0]) else np.nan,
        ...
        if inclusive:
            ...
            v_not_nan = v[not_nan]
            v[not_nan] = np.where(
                np.logical_and(v_not_nan >= range_[0], v_not_nan <= range_[1]),
                v_not_nan,
                np.nan,
            )
